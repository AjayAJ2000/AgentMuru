...
from agentmuru.server.auth import Principal, current_principal

logger = logging.getLogger("agentmuru.integrations.databricks.sql")
...
def _connect(*, principal: Optional[Principal] = None):
    ...
    try:
        db_sql = importlib.import_module("databricks.sql")
    except ImportError:
        raise ImportError(
            "databricks-sql-connector is required for SQL integration. "
            "Install it with: pip install agentmuru[databricks]"
        )
...
def query(sql: str, params: Optional[List[Any]] = None) -> "Any":
    ...
    try:
        ...
        raise ImportError("pandas is required. Install with: pip install agentmuru[databricks]")
